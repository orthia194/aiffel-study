...
## 🛡️ 과적합 방지 및 학습 안정화 기법 비교

| 항목 | **L1 정규화 (Lasso)** | **L2 정규화 (Ridge / Weight Decay)** | **드롭아웃 (Dropout)** | **배치 정규화 (Batch Normalization)** |
| :--- | :--- | :--- | :--- | :--- |
| **적용 위치** | Loss 계산식 (`loss + l1_lambda * l1_norm`) | Optimizer 설정 (`weight_decay=1e-4`) | Model 클래스 내 레이어 (`nn.Dropout`) | Model 클래스 내 레이어 (`nn.BatchNorm1d/2d`) |
| **제어 대상** | 가중치(Weight)의 절댓값 | 가중치(Weight)의 제곱 | 신경망 노드(Neuron) 간 연결 | 레이어 입/출력 데이터의 분포 (Mean & Variance) |
| **핵심 동작** | 중요도가 낮은 가중치를 **완전히 `0`으로 삭제** | 모든 가중치를 **전반적으로 작고 얌전하게 축소** | 학습 시 무작위로 **일부 노드를 끔 (OFF)** | 각 배치별 데이터를 **평균 0, 표준편차 1로 정규화** |
| **실무 사용 빈도** | 🔴 **낮음** (특수한 경우에만 사용) | 🟢 **매우 높음** (사실상 기본 적용) | 🟢 **매우 높음** (FC 레이어, 심층망 필수) | 🟢 **매우 높음** (CNN, 현대 딥러닝 필수) |
| **주요 사용 상황** | • 중요한 변수(Feature)만 선별하고 싶을 때<br>• 입력 데이터 종류가 너무 많고 듬성할 때 | • 모델의 급격한 가중치 변화를 막고 안정화할 때<br>• 기본적이고 전반적인 오버피팅을 방지할 때 | • 신경망이 깊고 복잡할 때 (FC Layer 등)<br>• 특정 노드 편중 현상(동조)을 방지할 때 | • 기울기 소실/폭주(Gradient Vanishing/Exploding) 방지<br>• 학습 속도를 획기적으로 올리고 안정화할 때 |
| **장점** | 불필요한 입력값을 완전히 제거하여 모델 경량화 | 가중치를 균일하게 관리하여 모델 안정성 향상 | 앙상블(Ensemble) 효과를 제공하여 실전 성능 대폭 증가 | 높은 학습률(Learning Rate) 사용 가능, 자체적 정규화 효과 |
| **단점** | 미분 불가능한 지점이 있어 학습 제어가 까다로움 | 가중치를 `0`으로 만들지는 못해 변수 선별 불가 | 학습 시간이 약간 늘어나며 평가 시 모드 전환 필수 | 배치 크기(Batch Size)의 영향을 많이 받음 (너무 작으면 불안정) |

---

### 📌 Summary

- **L2 Regularization + BatchNorm + Dropout**은 현대 PyTorch 모델에서 성능과 학습 안정성을 최대로 끌어올리기 위해 가장 광범위하게 쓰이는 조합입니다.
- **BatchNorm vs Dropout 조합 시 주의점:** 일반적으로 CNN 등 깊은 화상/음성 네트워크에서는 **`BatchNorm`**을 주력으로 사용하며, 마지막 전결합층(FC Layer)이나 특정 구조에서만 **`Dropout`**을 조합하여 사용하는 것이 표준적입니다.
