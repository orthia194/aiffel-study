...
- **개념**: 데이터 세트에서 값이 비어 있는(NaN, Null) 상태.
- **처리 방법**:
  - **삭제**: 결측치가 포함된 행(Row)이나 열(Column)을 제거 (데이터 손실 주의).
  - **대체 (Imputation)**: 대표값(평균, 중앙값, 최빈값)으로 비어 있는 칸을 채워 넣음.
- **핵심 규칙**: 결측치가 존재하는 상태로는 머신러닝/딥러닝 모델에 데이터를 입력할 수 없음.

---

### 2) 이상치 (Outliers)
- **개념**: 다른 데이터들과 비교했을 때 극단적으로 크거나 작은 '튀는 데이터'.
- **발생 원인**: 단순 입력 오류(측정 에러) 또는 실제 극단적 희귀 현상.
- **영향**: 평균이나 모델의 가중치를 왜곡시켜 전체적인 예측 성능을 떨어뜨림.
- **처리 방법**: IQR(사분위수 범위) 기법이나 Z-Score 등을 활용해 탐지 후 제거하거나 상/하한선으로 변환(Capping).

---

### 3) 정규화 & 스케일링 (Normalization & Feature Scaling)
- **개념**: 서로 다른 단위(예: '키 180cm'와 '몸무게 70kg', '소득 5,000,000원')를 가진 특성들의 수치 범위를 일정한 기준(0~1 사이 등)으로 맞추는 작업.
- **필요성**: 수치 단위가 큰 특성이 모델 학습을 주도해 버리는 현상을 방지함.
- **주요 기법**:
  - **Min-Max Scaling (정규화)**: 모든 수치를 **0과 1 사이**로 변환.
  - **Standardization (표준화)**: 평균을 0, 표준편차를 1로 만들어 **표준정규분포** 모양으로 변환.
- **주의할 점**: 전처리 기준(평균, 표준편차 등)은 반드시 **학습 데이터(Train Data) 기준으로만 fit**한 뒤, 테스트 데이터(Test Data)에는 적용(transform)만 해야 함 (데이터 누수 방지).
...
## 1. 그래프 그리기 기본 흐름
- **도화지에 그리기**: `plt.scatter(X, y)`
- **화면에 출력하기**: `plt.show()`
  - 그려진 그래프를 모니터에 실제로 띄워주는 필수 실행 버튼

## 2. 주요 그래프 4대 천왕
- **산점도 (Scatter Plot)**
  - `plt.scatter(x, y)`
  - 두 변수($X, y$) 간의 상관관계 및 데이터 분포 표현
- **선 그래프 (Line Plot)**
  - `plt.plot(x, y)`
  - 시간에 따른 추세 표현 (시계열 데이터, 주가, 기온)
- **막대 그래프 (Bar Chart)**
  - `plt.bar(x, height)`
  - 카테고리/범주별 크기 및 수치 비교 (성별 매출, 그룹별 평균)
- **히스토그램 (Histogram)**
  - `plt.hist(data)`
  - 데이터의 구간별 빈도수 및 분포 파악 (시험 점수, 연령대)

## 3. 히트맵 (Heatmap)
- **`sns.heatmap(data)`**
  - Seaborn 라이브러리 활용
  - 변수 간 상관관계를 색상의 짙고 옅음으로 한눈에 파악
